<template>
  <div class="container-custom py-8">
    ...
    <button @click="$router.back()" class="flex items-center gap-2 text-gray-700 hover:text-primary mb-4"
      aria-label="Regresar" type="button">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div class="col-span-12 md:col-span-6">
          ...
          <div class="hidden md:flex flex-row">
            ...
            <div
              class="bg-white rounded-lg shadow-md flex-1 flex items-center justify-center ml-4 relative max-w-2xl h-[500px]">
              <div class="w-full h-full relative flex items-center justify-center">
                ...
                <div v-else-if="activeMedia.type === 'video'" class="w-full h-full flex items-center justify-center">
                  <video :src="activeMedia.url" :alt="product.nombre" class="object-cover w-full h-full" autoplay muted
                    loop controls preload="metadata" @error="handleVideoError" @loadstart="handleVideoLoadStart"
                  ...
                  <div v-if="videoError" class="absolute inset-0 flex items-center justify-center bg-gray-100">
                    <div class="text-center p-4">
          ...
          <div class="md:hidden">
            ...
            <div class="relative bg-white rounded-lg shadow-md">
              <Swiper :modules="[Scrollbar]" :slides-per-view="1" :space-between="0" :scrollbar="{ draggable: true }"
                class="w-full h-[300px] rounded-lg" @slideChange="(swiper) => activeMediaIndex = swiper.activeIndex">
                <SwiperSlide v-for="(media, idx) in mediaItems" :key="idx">
                  ...
                  <NuxtImg v-if="media.type === 'image'" :src="media.url" :alt="product.nombre"
                    class="object-contain w-full h-full max-w-full max-h-full" />
                  <div v-else-if="activeMedia.type === 'video'" class="w-full h-full flex items-center justify-center">
                    <video :src="activeMedia.url" :alt="product.nombre" class="object-cover w-full h-full" autoplay
                      muted loop controls preload="metadata" @error="handleVideoError" @loadstart="handleVideoLoadStart"
                      crossorigin="anonymous">
                      <source :src="activeMedia.url" type="video/mp4">
                      Tu navegador no soporta el elemento de video.
                    </video>
                  ...
                  <div class="flex flex-col flex-1">
                    ...
                    <div class="flex border rounded-lg overflow-hidden w-full max-w-[200px] justify-between">
                      <button @click="cartQuantity = Math.max(getMinimumOrderQuantity() || 1, cartQuantity - 1)"
                        ...
                        class="w-12 h-12 flex items-center justify-center bg-gray-100 text-2xl font-bold disabled:opacity-50">-</button>
                      ...
                      <button @click="cartQuantity++"
                        class="w-12 h-12 flex items-center justify-center bg-gray-100 text-2xl font-bold">+</button>
                    </div>
                  </div>
                  <div class="flex flex-col flex-1">
                    <span class="font-semibold mb-1 text-base sm:text-lg">Total</span>
                    <div
                      class="w-full max-w-[200px] h-12 flex items-center justify-center border rounded-lg bg-gray-50 font-bold text-2xl sm:text-3xl">
              ...
              <div>
                <div class="flex flex-col gap-3 my-4">
                  <button v-if="!goToCart" @click="iniciarPedidoPanel"
                    :disabled="cartQuantity < getMinimumOrderQuantity()" :class="[
                  ...
                  <span v-if="cartQuantity < getMinimumOrderQuantity()"
              ...
              <li class="flex items-start gap-3">
                ...
                <span class="inline-block w-6 h-6 mx-1">
                  <svg width="25" height="28" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M14.7425 7.0412H12.4893C12.6566 6.70674 12.7622 6.33708 12.7622 5.94101V2.42041C12.7622 1.08258 11.6796 0 10.3418 0H6.82116C5.48333 0 4.40075 1.08258 4.40075 2.42041V5.94101C4.40075 6.33708 4.50637 6.70674 4.6736 7.0412H2.42041C1.08258 7.0412 0 8.12378 0 9.46161V13.8624C0 15.2002 1.08258 16.2828 2.42041 16.2828H6.82116C7.51648 16.2828 8.14139 15.9835 8.58146 15.517C9.02154 15.9923 9.64644 16.2828 10.3418 16.2828H14.7425C16.0803 16.2828 17.1629 15.2002 17.1629 13.8624V9.46161C17.1629 8.12378 16.0803 7.0412 14.7425 7.0412ZM5.72097 5.94101V2.42041C5.72097 1.81311 6.21386 1.32022 6.82116 1.32022H10.3418C10.9491 1.32022 11.4419 1.81311 11.4419 2.42041V5.94101C11.4419 6.54832 10.9491 7.0412 10.3418 7.0412H6.82116C6.21386 7.0412 5.72097 6.54832 5.72097 5.94101ZM6.82116 14.9625H2.42041C1.81311 14.9625 1.32022 14.4697 1.32022 13.8624V9.46161C1.32022 8.85431 1.81311 8.36142 2.42041 8.36142H6.82116C7.42846 8.36142 7.92135 8.85431 7.92135 9.46161V13.8624C7.92135 14.4697 7.42846 14.9625 6.82116 14.9625ZM15.8427 13.8624C15.8427 14.4697 15.3498 14.9625 14.7425 14.9625H10.3418C9.73446 14.9625 9.24157 14.4697 9.24157 13.8624V9.46161C9.24157 8.85431 9.73446 8.36142 10.3418 8.36142H14.7425C15.3498 8.36142 15.8427 8.85431 15.8427 9.46161V13.8624Z"
                      fill="#272A30" />
                    <path
                      d="M7.70237 5.28125H6.82222C6.46136 5.28125 6.16211 5.5805 6.16211 5.94136C6.16211 6.30222 6.46136 6.60147 6.82222 6.60147H7.70237C8.06323 6.60147 8.36248 6.30222 8.36248 5.94136C8.36248 5.5805 8.06323 5.28125 7.70237 5.28125Z"
                      fill="#272A30" />
                    <path
                      d="M3.30198 13.1992H2.42183C2.06097 13.1992 1.76172 13.4985 1.76172 13.8593C1.76172 14.2202 2.06097 14.5194 2.42183 14.5194H3.30198C3.66284 14.5194 3.96209 14.2202 3.96209 13.8593C3.96209 13.4985 3.66284 13.1992 3.30198 13.1992Z"
                      fill="#272A30" />
                    <path
                      d="M11.2219 13.1992H10.3418C9.98089 13.1992 9.68164 13.4985 9.68164 13.8593C9.68164 14.2202 9.98089 14.5194 10.3418 14.5194H11.2219C11.5828 14.5194 11.882 14.2202 11.882 13.8593C11.882 13.4985 11.5828 13.1992 11.2219 13.1992Z"
                      fill="#272A30" />
                  </svg>
                ...
                <div>
                  ...
                  <span class="text-gray-500 text-sm">No se acepta pedidos por unidades, compra mínima de
                    <b>s/3.000</b></span>
                </div>
              </li>
        ...
        <div class="block md:hidden">
          ...
          <ul class="space-y-3 py-4">
            ...
            <li class="flex items-start gap-3">
              <!-- Icono tiempo -->
              <span class="inline-block w-6 h-6 mx-1">
                <svg width="25" height="28" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M8.58146 0C3.84625 0 0 3.84625 0 8.58146C0 13.3167 3.84625 17.1629 8.58146 17.1629C13.3167 17.1629 17.1629 13.3167 17.1629 8.58146C17.1629 3.84625 13.3167 0 8.58146 0ZM9.24157 15.8075V14.7425C9.24157 14.3816 8.94232 14.0824 8.58146 14.0824C8.2206 14.0824 7.92135 14.3816 7.92135 14.7425V15.8075C4.44476 15.4906 1.67228 12.7182 1.35543 9.24157H2.42041C2.78127 9.24157 3.08052 8.94232 3.08052 8.58146C3.08052 8.2206 2.78127 7.92135 2.42041 7.92135H1.35543C1.67228 4.44476 4.44476 1.67228 7.92135 1.35543V2.42041C7.92135 2.78127 8.2206 3.08052 8.58146 3.08052C8.94232 3.08052 9.24157 2.78127 9.24157 2.42041V1.35543C12.7182 1.67228 15.4906 4.44476 15.8075 7.92135H14.7425C14.3816 7.92135 14.0824 8.2206 14.0824 8.58146C14.0824 8.94232 14.3816 9.24157 14.7425 9.24157H15.8075C15.4906 12.7182 12.7182 15.4906 9.24157 15.8075Z"
                    fill="#272A30" />
                  <path
